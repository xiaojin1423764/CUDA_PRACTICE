...
- **下一轮优化：如何减少同步和 launch 开销？** 首选将一维全局 softmax 重构为实际模型常用的 row-wise softmax：一个 block 负责一行，在 block 内用 warp shuffle 和 shared memory 依次完成 `max -> exp/sum -> normalize`，将当前 5 个 kernel 和跨 kernel 全局同步缩减为每行 1 个 kernel。对无法由一个 block 容纳的超长单行，保留 partial reduction，但继续合并 `exp` 与 partial sum、复用中间结果；若 launch 开销仍占主导，再评估 CUDA Graph，或在 cooperative launch 的占用限制允许时使用 grid 级同步。
